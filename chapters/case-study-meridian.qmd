...
The root cause pointed to a workflow and a knowledge gap — not a person. Elena was the symptom. The missing system was the constraint.

### Source

Elena cleared her Thursday afternoon. She sat down with Mark, Ty, and the Compound partner running the Sprint, and they walked through what she actually knew, where it lived, and what was missing.
...
The design session ran on a Friday morning. Elena, Mark, Ty, Dave, and the Compound partner sat in the conference room with the constraint statement and Knowledge Map printed on the whiteboard.

The first question was the sorting question: which parts of the quoting process genuinely require Elena's judgment, and which parts require data she happens to hold?
...
The remaining 35% was genuinely judgment-heavy: evaluating whether a new RFQ was similar enough to a historical job to use as a pricing reference, deciding how to handle a spec that pushed the shop's capabilities, and making the call on custom pricing for strategic accounts.
...
The Compound partner reframed: "What if you reviewed every estimate before it went out? The agent does the assembly — pulls the historical data, applies the rate card, formats the document — and you review it in fifteen minutes instead of building it from scratch in three hours."

Elena paused. "That's different. If I'm reviewing it, that's different."

That exchange defined the design. AI-assisted, not automated. Elena reviews every output. The agent handles the 65% that's data retrieval and assembly. Elena handles the 35% that's judgment. And the turnaround drops from days to hours because Elena's reviewing a draft rather than building from scratch.
...
Elena's role didn't shrink. It shifted. She went from building every quote to reviewing agent-generated drafts and spending the recovered hours on the strategic work that had been perpetually deferred — capacity planning, supplier renegotiations, and the process improvements the shop needed.
...
**Human Orchestrator:** Elena Ruiz, VP of Operations. Owns the constraint outcome: reduce quote turnaround from 3-5 days to same-day for standard work.

**Agent Coordinator:** Ty Banfield, Sales Lead. Monitors intake quality, flags quotes stuck in the pipeline, reports on turnaround times weekly.
...
- [x] The Human Orchestrator (Elena) and Agent Coordinator (Ty) are named — with irreducible decisions and escalation triggers documented
...
The build path was low-code. The team used Claude as the AI backbone — a Claude Team workspace with three configured projects (one per agent), each loaded with the relevant knowledge from Source. The workflow orchestration ran through n8n, which Meridian already had a license for from a previous automation attempt that never went anywhere. The CRM (HubSpot) and ERP (JobBOSS) both had APIs.

The build took three and a half weeks. The Compound partner handled the technical assembly. Elena spent four hours in structured interviews — what the Source chapter calls the transcript extraction — walking through her pricing exception logic, her material estimation heuristics, and her process for evaluating RFQ complexity. Those transcripts became the core knowledge base for the Quote Research and Quote Pricing agents.
...
| **Tools/platforms** | Claude Team (3 agent projects), n8n (workflow triggers and routing), HubSpot API, JobBOSS API, Google Workspace (document generation and storage) |
...
The intake form captured the specs. The Quote Research Agent pulled the customer's history: twelve previous jobs over four years, three of them bracket work. It matched against historical jobs and surfaced the closest comparables with pricing. The Quote Pricing Agent applied the rate card, applied the customer's documented 8% volume discount, and produced a draft at \$14,200. Elena received the draft in her review queue eleven minutes after the RFQ was submitted.
...
The two new Greens — Workflow Visibility and Measurement Discipline — are permanent. The quoting workflow is documented and designed. The metrics are tracked. These don't slide back. They're infrastructure.
