...
The Audit checks what Build prepared — it doesn't author anything fresh. If any answer is missing, that item blocks the deploy. Failed deploys trace back to a question nobody asked at this gate.
...
Deliver also surfaces what did not work. This isn't a postmortem — the retrospective belongs to Compound, and that's the next chapter. This is the operational log that captures, in real time, the gaps between what was designed and what production actually revealed.
...
That's the clean case. The interesting cases are the ones where the measurement is harder. The sprint clearly changed the work — the operator's week looks different, the bottleneck is gone, the log shows unplanned interventions dropping week over week — but the dollar figure is hard to pin down because the original Signal quantification was looser than it should have been. That's not a Deliver failure. That's a Signal lesson, and it goes into the Compound stage as an input to how the next sprint gets framed.
...
That artifact set — the outcome, the log, the runbook, the updated Hybrid Accountability Chart entry — is what the next stage works against. Compound is where the team turns it into the inputs that accelerate the next sprint. That's the next chapter.
