...
Gate 4 names the Human Orchestrator — the person accountable for reviewing, course-correcting, and closing the loop on what the agent team produces, sprint by sprint (not a chatbot supervisor, but an operational owner with a weekly cadence; covered in depth in the Design chapter). If you run EOS, think of it as the Integrator role extended to cover the agent team — the scope is different (Sprint-level, not company-level), but the accountability pattern is the same.
