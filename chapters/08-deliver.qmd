...
<!-- FORMAT: table -->
<!-- PRIORITY: must-have --> You are not writing a postmortem — you are leaving a trail that Compound can read when it asks: what would we do differently next time?
...
The Audit's job is to verify these answers exist before the switch flips — it checks what Build prepared, it does not author it fresh. If any answer is missing, that item blocks the deploy. You resolve it first, then re-run the check. Most failed deploys can be traced back to a question that nobody asked at this gate.
...
<!-- FORMAT: table -->
<!-- PRIORITY: must-have --> Get it in the tool they already live in — the project management system, the shared doc, the Slack channel — before the workflow goes live, not after.
