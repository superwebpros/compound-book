...
{{< excalidraw ch08-readiness-gates-progressive-1 "The Deploy Readiness Audit clearing, gate by gate, with Meridian's answers: Gate 1 passes on hands-on training, three gates still pending." >}}

{{< excalidraw ch08-readiness-gates-progressive-2 "Gate 2 clears — the escalation path was already written and published before the audit began." >}}

{{< excalidraw ch08-readiness-gates-progressive-3 "Gate 3 clears — the workflow tested against two weeks of real RFQs, not demo data." >}}

{{< excalidraw ch08-readiness-gates-progressive-4 "Gate 4 clears — the Human Orchestrator's review cadence is on the calendar. Four gates, four passes: 4/4, clear to deploy." >}}
...
The heuristic: if nothing in the test set makes your best person wince, you tested the demo, not the operation.
...
The cadence tapers on evidence, not on time. The target Designing the Work set is to earn Medium through demonstrated accuracy over multiple Sprints, and the operational log below is where that evidence accumulates. The review never comes off the calendar entirely; that's the difference between earned trust and quiet abandonment.
...
Elena didn't treat the audit as paperwork. For Gate 1 she ran real quotes through the pipeline herself while Dave sat with the agent's historical job matching, checking its picks against specs he knew by heart. That was the hands-on practice: live work, not a walkthrough. Gate 2 was already written — the escalation path sat in the Claude Team workspace, non-standard materials routing to Dave, low-confidence quotes routing to Elena for full manual review, every assembled quote passing through her before it reached a customer. For Gate 3 they pulled real RFQs from HubSpot, actual customer requests from the prior two weeks, and ran them end to end: HubSpot connected, JobBOSS connected, Customer Notes.xlsx with its 112 validated pricing rules loaded into the pricing project's context. Gate 4 went on the calendar as Monday and Thursday mornings, recurring. Four gates, four passes. Clear to deploy.

:::{.callout-note}
## Action Step
Pull up your Hybrid Accountability Chart from Designing the System — the map of who owns what, extended to include the agent roles alongside the human ones. Set it next to the Design Brief's role-change list and confirm every person whose daily work changes has completed hands-on training that meets Gate 1's bar. If anyone hasn't, that's your first task before deploy.
:::
...
{{< excalidraw ch08-per-role-runbook "The per-role runbook: one card per affected role — what they receive, what they produce, and when they escalate. Repeat the pattern for every role whose handoffs changed." >}}
...
The heuristic: if the sentence could be pasted into another role's row without anyone noticing, it isn't concrete enough.
...
{{< excalidraw ch08-operational-log-blank "Operational log, Day One (empty): headers set, the four locked categories keyed below — Unplanned intervention, Data gap, Edge case, User confusion — no rows yet." >}}
...
{{< excalidraw ch08-operational-log-day2 "Day 2 — the operational log's first entry: the Inconel edge case, caught in review and closed with a same-day guardrail." >}}

{{< excalidraw ch08-operational-log-day4 "Day 4 — a second entry joins: the missing-drawing user confusion, closed with a required intake field." >}}

{{< excalidraw ch08-operational-log-day7 "Day 7 — a third entry joins: Dave's tolerance catch, filed as an unplanned intervention and carried forward as a design task." >}}

{{< excalidraw ch08-operational-log-day10 "Day 10 — the fourth entry (a positive signal) and the week-two tally: three gap categories, one entry each, with the still-open one — tolerance class as a matching criterion — pointing at the next Sprint's candidate constraint." >}}
...
:::{.callout-note}
## Action Step
Create the log template in whatever tool your team already uses, with four columns: Date, What happened, Category, What it signals. Name the person who owns it. Do this before you flip the switch, not after.
:::
