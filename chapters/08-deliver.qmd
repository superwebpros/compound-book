...
Both categories get written down. Not in a formal incident-report shape — in a running log the Agent Coordinator maintains as part of the operating rhythm. That log is operational: it captures what happened in real time, as the sprint runs in production. It is distinct from the documented sprint outcome Deliver produces at the close — the summary of what the sprint changed, measured against Signal's quantified cost — which is the artifact Compound works against when it asks: what would we do differently next time?
...
The **Deploy Readiness Audit** is the pre-flight check. Are the people whose handoffs change trained on the new flow? Are the inputs to the agent clean and arriving in the expected format? Are the escalation paths defined, named, and acknowledged by the humans who own them? Does the rollback plan Build's Guardrails Checklist produced contain the specific steps the team takes if the new workflow produces results the organization cannot accept, and is it clear who has the authority to invoke it? The Audit's job is to verify these answers exist before the switch flips — it checks what Build prepared, it does not author it fresh. Most failed deploys can be traced back to a question that nobody asked at this gate.
