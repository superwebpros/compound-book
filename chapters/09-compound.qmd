...
Each Sprint I ran built on the last. The agents (software that holds a goal and runs a sequence of steps on its own — not a chatbot you prompt one question at a time; covered in depth in the Build chapter) knew the company's conventions because they'd learned them in an earlier Sprint. The tools knew the formats because we'd built the templates. The Knowledge Maps from the first Sprint were sitting inside the system, accelerating the fifth. None of that had to be rebuilt. It just ran.
...
The Compound session isn't the quarterly operating session you'll meet in the next chapter, and the split matters: the Compound session closes the Sprint and nominates the next one (the governing constraint plus a proposed Human Orchestrator); the quarterly operating session commits it (constraint, Human Orchestrator, Sprint Lead, budget, start date).
...
- An updated **Constraint Backlog**: the running, prioritized list of validated constraints your organization is working through, re-ranked against what the Sprint just taught you.
...
:::{.callout-note}
## Action Step
Before the Sprint enters Build, put the Compound session on the calendar. Include the Sprint Lead (who runs the session), the Human Orchestrator (the operator who runs the redesigned workflow and supervises its agents day-to-day — detailed in the Designing the System chapter), and everyone who owned a Sprint accountability. If it's not scheduled before the Sprint ships, it won't happen after.
:::
...
This is the retrospective's closing move, the fifth step in the how-to list above. Beyond the Sprint outcome you measured in Deliver, Compound asks three more questions. The answers are the compound interest.
...
4. **Install the change structurally before the next Sprint begins** — The change must land in the chart, the workflow, or the Knowledge Map; a change that lives only in a document changes nothing.
...
The picking heuristic: choose the change the next Sprint would trip over first. Not the biggest observation and not the easiest fix. The one standing in the next Sprint's path.
...
*Did the Sprint reveal new information about any of the other constraints?* Solving one constraint frequently exposes the real cost of another. The workflow you just fixed was hiding a manual re-entry step downstream. The report you automated revealed that its data source updates weekly, not daily. Expect at least one ranking to move; a Sprint that changes nothing about the backlog usually means nobody looked.
...
*Did any new constraints surface during the Sprint that weren't on the original list?* A Sprint is a discovery engine. The team that lived inside a workflow for the length of the Sprint saw things no Signal session would have surfaced. Capture them now, at the moment of highest clarity. A constraint that waits for the next Signal session usually arrives without its evidence.
