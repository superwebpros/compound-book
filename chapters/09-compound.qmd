...
Compound is the sixth stage of the Sequence. It turns a delivered Sprint into the foundation of the next one. Running the Sequence on a cadence makes the work compound, which is where our company name comes from.
...
If your Sprint ends and the backlog isn't re-ranked, the chart hasn't been updated, and no next constraint is named, the Sprint didn't finish. Compound is the stage where those three things get done.
...
*What does the organization know now that it didn't know before this Sprint?* Meridian learned that Elena's Customer Notes spreadsheet, 147 rows of pricing exceptions on her desktop, contained only 112 validated rules. Thirty-five were duplicates or outdated. They learned which quote types the agent handles cleanly (standard specs, about 70% of volume) and which still need Elena's review (complex fabrication, specialty alloys). Before the Sprint, they treated every quote as equally hard. Now they know it isn't.
...
The discipline of one design change defeats that failure mode. The Human Orchestrator looks across everything the Sprint surfaced: the Source phase that ran too thin, the supervisor cadence that was wrong, the handoff between the agent team and the human reviewer that broke once, the input format that turned out to need cleaning nobody scoped. Then the Orchestrator names the *single* change that will most improve the next Sprint.
...
One design change per Sprint is small. Eight Sprints of one good design change each is substantial. That's the Co-Operating Model: across the Sprints, it gets better at running the Sequence. Disciplined improvement over many Sprints produces an agent team dramatically more capable after eight Sprints than after one. Skip the discipline and you get the same agent team you always had.
...
This is the structural payoff of the whole Sequence. This work gets cheaper and better with each Sprint.
...
*Compound* is the name of the company. It's the verb the work produces. It's the stage of the Sequence you're reading about now.

Compound the stage is where the verb happens. The previous Sprint's output feeds the next Sprint. That mechanism is what Compound the company exists to install. The brand and the stage share a name because the company *is* the stage. The company teaches you to run this last step, and to run it every time.
