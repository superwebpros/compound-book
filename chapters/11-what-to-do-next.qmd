...
The Human Orchestrator is the person who owns the constraint outcome and supervises the agent team — not a technical role, not a new hire, but the operator who already owns that workflow and can judge whether the agent's output is right (more on this role in the Designing the System chapter). If you run EOS, think of the Integrator role extended to a team that includes agents — the Orchestrator runs the Sprint the way the Integrator runs the company, but the scope is one workflow, not the whole org. The Hybrid Accountability Chart (if you run EOS: your Accountability Chart extended to include agent rows alongside the human ones) is where that role lands on paper.
