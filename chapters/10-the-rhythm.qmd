...
Every company running the Rhythm maintains one living document: the Hybrid Org Today. It is the current operational picture of the Co-Intelligent Company.
...
After six to eight sprints on the Rhythm, the math moves. Not overnight — but the pattern is broken. The behavioral marker is specific. The CEO's first response to a capacity problem is no longer *who do we hire?* It is *what does Signal say about this?*

That is the shift. Some constraints are headcount constraints, and the answer is to hire. The shift is that the question gets asked with the Signal Agent, against the Signal Backlog, *before* the requisition is written. And more often than the organization expected, the answer comes back that the constraint is a design problem, not a headcount problem — and the sprint that addresses it ships a designed workflow instead of a job posting.
