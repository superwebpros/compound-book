...
**Step 3: Commit the next Sprint.** Pick the top constraint from the updated backlog. Name the Human Orchestrator (the person on your leadership team who owns the Sprint's output and supervises the agent team doing the work — the Design chapter covers how this role is assigned; if you run EOS, think of this as the Integrator role extended to the Sprint level, not the whole company). Confirm the team has the resources — people, tools, budget. Set the Sprint start date. One validated constraint. One constraint owner. A specific six-to-eight-week window. The one-page Constraint Statement from Signal is the artifact. Your team leaves the session knowing exactly what the next Sprint is about and who owns it.
...
3. Your quarterly operating session has four steps: review the previous Sprint at the outcome level, update the Signal Backlog, commit the next Sprint, and update the Hybrid Accountability Chart. Is that session on your calendar this quarter — with all four steps on the agenda and the right people in the room?
