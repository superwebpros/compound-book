...
8. **Environment and constraints.** Which specific environment inside Design's category (off-the-shelf, low-code, hand-built — see Chapter 7) this build runs on, and why. Any constraints the builder must observe — data residency, existing licenses, internal security requirements.
...
Before you finalize the environment decision, check the agent count. Per Bedard's three-agent ceiling (see Chapter 7), productivity inverts after three concurrent agents under one supervisor. Up to three, adding agents produces compounding gains. Beyond three, the supervisor's cognitive budget runs out and error rates climb.
