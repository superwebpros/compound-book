...
:::{.callout-important}
## In Brief
Build is the stage where the locked Design Brief becomes a working, deployed system. Design has already made the hard calls: which constraint to solve, which workflow to build, which platform category fits. Build's job is narrower. It produces one thing: a deployed solution to the validated constraint, specified by an eight-section Build Spec, stress-tested against a seven-question Guardrails Checklist, and confirmed done only when it handles real work under real supervision. Once the spec and guardrails are locked, there's one question left: where does this agent actually live? That's the platform-decision — and it lives here, not in Design. If Build feels slow or chaotic, the problem is almost never in Build. It is upstream.
:::
...
This is the platform decision, and it belongs in Build — not Design. Design picked a category: low-code workflow, shared AI environment, custom integration. Build picks the specific platform inside that category and configures the agent to run there. The distinction matters because platform options shift faster than workflow design principles do. What's named here is the category logic; the current products are examples. Check the market when you're running your Sprint, not when you read this.

The categories, with what fits and what doesn't:

**Claude Projects** fit skill-level work: a single agent, a defined knowledge base, one person or a small team using it co-operatively. The "build" is configuration — a system prompt, uploaded context, defined scope, guardrails set in the project settings. A competent person does this in an afternoon. This is the right environment when the designed workflow is one agent, one output stream, one supervisor reviewing every output. The marketing lead's four-agent setup from Chapter 3 started here. When a workflow grows past one agent or starts running programmatically, Claude Projects doesn't stretch to cover it.
...
### The span-of-control constraint.

Before you finalize the platform decision, check the agent count against a finding from Bedard et al. (BCG, HBR, March 2026): productivity inverts after three concurrent agents under one supervisor. Up to three, adding agents produces compounding gains. Beyond three, the supervisor's cognitive budget runs out and error rates climb.

The finding named it "AI brain fry" — mental fatigue from oversight load exceeding a person's capacity. The practical implication for Build: if the designed system puts four, five, or six agents under one human supervisor, the design has a structural problem. The platform decision doesn't fix that. You can run six agents on the best infrastructure available and still produce worse outcomes than three agents on a simpler setup, because the bottleneck is the supervisor's attention, not the platform's throughput.

If the platform decision is pushing toward a many-agent architecture, stop. Go back to Design. Either scope the system smaller (fewer agents per supervisor) or add a layer of agent-supervised parallel work, where a coordinator agent manages the subagents and the human supervisor reviews the coordinator's output rather than every individual agent's output. That pattern is named in Chapter 3: the quote-orchestrator that runs a team inside its own loop, with the human reviewing what the orchestrator surfaces, not what each subagent produced. Chapter 7, Design, is where that architectural decision gets made. Build inherits it.

The platform decision should match the architecture Design produced, not the architecture the builder finds interesting.

:::{.callout-note}
## Action Step
Map your designed agent count against the supervisor named in the Hybrid Accountability Chart. If the count exceeds three agents under one supervisor, flag it as a design gap before Build proceeds. The platform you pick won't solve a supervision problem; only the design can.
:::
