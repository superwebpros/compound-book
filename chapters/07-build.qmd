...
:::{.callout-important}
## In Brief
Build is the stage where the locked Design Brief becomes a working, deployed system. Design has already made the hard calls: which constraint to solve, which workflow to build, which category fits. Build's job is narrower. It produces one thing: a deployed solution to the validated constraint, specified by an eight-section Build Spec, stress-tested against a seven-question Guardrails Checklist, and confirmed done only when it handles real work under real supervision. Once the spec and guardrails are locked, there's one question left: where does this agent (an AI system that holds a goal, runs a sequence of steps toward it, and adjusts based on its own outputs — not a chatbot answering one question at a time) actually live? That's the environment decision — and it lives here, not in Design. If Build feels slow or chaotic, the problem is almost never in Build. It is upstream.
:::
...
The real requirement is curiosity and systems thinking. The rest is teachable. I teach psychology, economics, and criminal justice majors at Michigan State, not engineers or computer scientists. Within a semester, those students are building functional data pipelines. Pulling data from APIs (application programming interfaces, how systems talk to other systems), transforming it through agents, writing it back to systems. They learned because the tools have been designed to make high-value work accessible to non-engineers. Your team will have to learn new tools. Some of them will have to start thinking in systems. That is the bar. A computer science degree isn't.
...
You (the CEO or operator) decide model, deployment, and data-access approach. The builder handles configuration, APIs, and the wiring underneath. The sections below give you the vocabulary you need to make those calls.
...
You decide which of the three options below the workflow needs. The builder handles the hosting, monitoring, and runtime details once you pick.
...
The Framework chapter's Compound Sprint is the strategic container. Build's Sprint discipline is the tactical version — same principle of time-boxed, scoped, accountable work, applied to the construction of a single workflow.
...
8. **Environment and constraints.** Which specific environment inside Design's category (off-the-shelf, low-code, hand-built — see Designing the System) this build runs on, and why. Any constraints the builder must observe — data residency, existing licenses, internal security requirements.
...
Here is the blank seven-row Guardrails Checklist template. Answer every question in writing before Build executes. If any question cannot be answered, the build is not ready.
...
Before you finalize the environment decision, check the agent count. Per Bedard's three-agent ceiling (see Designing the System), productivity inverts after three concurrent agents under one supervisor. Up to three, adding agents produces compounding gains. Beyond three, the supervisor's cognitive budget runs out and error rates climb.

The practical implication for Build: if the designed system puts four, five, or six agents under one human supervisor, the design has a structural problem. The environment decision doesn't fix that. You can run six agents on the best infrastructure available and still produce worse outcomes than three agents on a simpler setup, because the bottleneck is the supervisor's attention, not the environment's throughput.

If the environment decision is pushing toward a many-agent architecture, stop. Go back to Design. Either scope the system smaller (fewer agents per supervisor) or add a layer of agent-supervised parallel work, where a coordinator agent manages the subagents and the human supervisor reviews the coordinator's output rather than every individual agent's output. That pattern is named in the Co-Operating Model chapter: the quote-orchestrator that runs a team inside its own loop, with the human reviewing what the orchestrator surfaces, not what each subagent produced. Designing the System is where that architectural decision gets made. Build inherits it.

The environment decision should match the architecture Design produced, not the architecture the builder finds interesting.
