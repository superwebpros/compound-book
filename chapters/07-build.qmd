...
*Working* means the solution has been tested against real inputs (the actual records, emails, quote requests, customer files, whatever the design specified) and produces outputs the Human Orchestrator can review and trust. The outputs land in a form the Human Orchestrator can evaluate the same way they would evaluate a person's work.
...
That bar is deliberately high. Build is done when work the team used to do is now being done by the designed system, supervised by the Human Orchestrator the Hybrid Accountability Chart names. A demo that impressed leadership doesn't clear it, and neither does a prototype sitting in a sandbox. A workflow that works but nobody uses is not done.
...
- [ ] **Check 1: Search for existing capabilities before building.** Search before you build. Does a tool, feature, or integration already exist that does what this section of the spec describes? Fifteen minutes of searching saves weeks of building. This is the most common implementation failure, and it's not technical. It's a research failure.
- [ ] **Check 2: Confirm the environment matches Design's category.** Match the environment to Design's category, not the other way around. If the spec calls for low-code and someone proposes a custom API integration because "it would be cleaner," the spec wins.
- [ ] **Check 3: Verify the spec is complete before build starts.** Every empty section in the spec is a decision the builder will make on the fly. Builders aren't designers. The decisions they make under build pressure won't match what Design intended.
- [ ] **Check 4: Confirm the output lands where the work already lives.** The output must land where the work already lives. If the output lands in a new tool the team has never opened, adoption is zero.
- [ ] **Check 5: Verify Section 7 (failure modes) is populated.** If Section 7 is empty, the build handles the happy path and breaks on the first edge case. Every workflow has edge cases.
- [ ] **Check 6: Confirm the build handles real-world edge cases, not just the demo path.** A demo shows the happy path. A build handles the edge cases, the missing data, the system outages, the weird inputs, the Human Orchestrator who's on vacation.
- [ ] **Check 7: Require testing against real inputs before declaring done.** Pull last week's actual data, the real records and the real requests, and run them through the build. Synthetic data and cherry-picked examples aren't tests.
...
These are the conditions under which the Human Orchestrator can actually supervise.
...
|---|---|---|---|
| **Claude Projects** | One agent, one knowledge base, one Human Orchestrator; configuration in an afternoon | More than one agent, or anything that runs programmatically | Marketing lead's four-agent setup |
...
Before you finalize the environment decision, check the agent count. Per Bedard's three-agent ceiling (see the Designing the System chapter), productivity inverts after three concurrent agents under one Human Orchestrator. If the designed system puts four, five, or six agents under one Human Orchestrator, the environment decision won't fix that. Go back to Design.
