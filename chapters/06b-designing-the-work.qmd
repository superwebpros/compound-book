...
**2. Tools.** What the agent is allowed to call: APIs, search, file access, integrations. Name every one. If a tool isn't listed, the agent doesn't get it.
...
When your Hybrid Accountability Chart assigns more than three concurrent agents to one supervisor, that supervisor needs to justify why the load doesn't violate the productivity-inversion threshold Bedard et al. identified: after three concurrent AI tools, productivity scores dipped. The three-agent ceiling isn't a hard stop, but it's a design question you answer explicitly. If Elena supervises four agents, what is the review structure that prevents cognitive overload? If the answer is "she'll manage," the design isn't finished.
...
The Design Brief feeds the Build Spec in the next chapter. The Build Spec isn't a parallel artifact; it's derived from this one. Where the Design Brief says *what* the system should do and *why*, the Build Spec restates the relevant sections at developer-execution depth and adds the three things Design doesn't own: agent scope at the call level, failure-mode behavior, and the build path itself. A gap in the Design Brief becomes a question in Build. Close the gaps here.
