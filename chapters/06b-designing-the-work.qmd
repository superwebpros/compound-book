...
The Design Brief's hardest job is specifying each agent with enough precision that Build can execute without guessing. For every agent named in your Hybrid Accountability Chart, the Design Brief includes a mini-spec covering six fields. These six come from the agent-anatomy concept introduced in the Co-Operating Model chapter: system prompt, tools, context sources, memory rules, judgment and escalation rules, and oversight load.
...
When your Hybrid Accountability Chart assigns more than three concurrent agents to one supervisor, that supervisor needs to justify why the load doesn't violate the productivity-inversion threshold. Per Bedard's three-agent ceiling (from Designing the System), the three-agent ceiling isn't a hard stop, but it's a design question you answer explicitly. If Elena supervises four agents, what is the review structure that prevents cognitive overload? If the answer is "she'll manage," the design isn't finished.
