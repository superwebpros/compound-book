...
The project coordinator story — the one where we mapped the work instead of posting a job — walks all six stages of the Framework. You will see it in full in Chapter 6. Here, it serves as proof that the sequence works when followed in order: Signal named the constraint, Source mapped what the role actually required, Design allocated the work between human and agent, Build produced the workflow, Deliver shipped it into the team's environment, and Compound captured what the next Sprint would inherit. One constraint. Six stages. One documented outcome.
