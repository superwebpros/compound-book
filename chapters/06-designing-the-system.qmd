...
The artifact that comes out of the Design Team's work is a *Design Brief* — the single document that captures the design decisions, stakeholders, systems, and specifications Build will inherit. The next chapter introduces the Design Brief in detail. Chapter 7 then shows how the Build Spec is derived from it: restating the relevant sections at developer-execution depth, and adding the implementation-level content (agent scope, failure modes, build path) that Design does not own.
