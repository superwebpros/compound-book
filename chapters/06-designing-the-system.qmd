...
What does ramp-up look like? The first sprint is supervised — the Orchestrator runs Design with guidance, either from a Compound partner or from the Bench agents that enforce the same question sequence. By the second sprint, they're running the process. By the fourth, they're improving it. The skill isn't learned in a training session. It's learned by doing the work, one sprint at a time, with the framework holding the structure while the person builds the muscle.
...
That's one constraint, one sprint, five chart rows, two named humans, and five guardrail answers. The next chapter covers the instruments and execution mechanics — Work Deconstruction, the Design Agent on the Bench, and the gate checklist that separates Design from Build.
