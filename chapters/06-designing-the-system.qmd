...
:::{.callout-important}
## In Brief
Design is the stage where your constraint and knowledge map become a workflow with named owners — human and AI. It produces four things: Hybrid Accountability Chart entries that assign every accountability to a named supervisor; a governance framework (autonomy levels, guardrails, escalation paths) that makes the system safe to operate; a mini-spec for every agent on the chart (an agent is an AI system that holds a goal, runs a sequence of steps toward it, and adjusts based on its own outputs — not a chatbot answering one question at a time); and a category choice for each agent before Build begins. This chapter covers the frameworks for designing the system. The next chapter covers designing the work itself.
:::
...
Design has four deliverables that land together.
...
The third is a *governance framework* for every agent team on the chart: autonomy levels, guardrails, and escalation paths. The fourth is a *mini-spec* for every agent and a *category choice* for each one before Build begins.
...
The Hybrid Accountability Chart is inspired by that structure and adds rows for agent teams. If you run EOS, you already have an Accountability Chart — the HAC isn't a replacement for it, it's a structural addition that operates alongside it. Every row still names one accountability and one human supervisor. But now some rows also name the agent team doing the execution work underneath that supervisor. The accountability doesn't move from human to agent. The ownership stays with the supervisor. What changes is who does the execution work, and the chart makes that visible.
...
Every accountability in a knowledge business is an information problem. The project coordinator takes information from one system, combines it with information from another system, applies rules she carries in her head, and produces an output that moves to the next person in the chain. The quoting process pulls customer data from the CRM, matches it against pricing tables in the ERP, applies exception rules that live in a spreadsheet on someone's desktop, and assembles the result into a format the customer can read. When you see the work as information flow, the design question changes. The question becomes: what information does this work need, where does it live, and how should it move?
...
A note on terms before we go further. Human Orchestrator is a Sprint-level role: the person who sets goals for an agent team, designs the workflow, and owns the constraint outcome for one Sprint. If your company runs EOS, the Integrator is a full-company role that runs day-to-day operations across every function. The Integrator may be the Human Orchestrator on a given Sprint, but the roles are not equivalent. The Integrator's scope is the whole company. The Orchestrator's scope is one workflow.
