...
```jf-note:
About the sources of information, we actually asked about the role and we separated out the parts that were human task are required human oversight, and then the parts that were just about data flows. The way we position this sounds like it was just about sources, but it was more than that; it was sources and _outcomes._
```
...
The four questions are not a template to fill in once and forget. They are a design discipline. Every sprint adds one row `jf-note: adds a row? to what?`, and every row forces the four answers — including the harder ones. "Who supervises this team?" is uncomfortable when the agent team is producing work nobody on the leadership team has owned before. That discomfort is the point. The chart surfaces it before any code is written.
...
```jf-note:
To think of a way to provide some prompts that will help lead people through understanding these things that they can copy and put into ChatGPT or Claude or something.
```
...
```jf-note:
I think at the end we're gonna need a summary table on when to use what people aren't gonna have a muscle for knowing when to pick which item it's not obvious
```
...
{{< excalidraw ch06-ai-assisted-automated-spectrum "AI-assisted to Automated spectrum: decision triggers at each pole, with sprint progression example" >}}
...
This is where most operators go wrong. They build the agent team first and figure out permissions later. `jf-note: I know i've done this...im not sure that most companies with IT departments do this. in fact, they're more likely to be slowed by IT stalling this before anything gets built. Blessing and a curse...` By "later" I mean after the wrong person has pulled data they shouldn't have, or the agent has drafted a client-facing email nobody reviewed. Governance is not a compliance layer you add at the end. It's a design decision you make before Build begins.
...
Design produces a workflow and a chart entry `jf-note: what does this mean?`. It also produces a role: the *Human Orchestrator* for this accountability.
...
```jf-note:
I think one of the things we believe is that everyone will eventually need to become an orchestrate at some level so we should definitely talk about the skills involved in being an orchestrated, chiefly context management, knowing what good looks like, and the ability to communicate clearly across different agents in different platforms.. and a working understanding of the tools and information different agents need to be successful. It's essentially not that different than good management great conditions for the people you're responsible for to be successful...
```
...
Beneath the Human Orchestrator sits an operational role: the *Agent Coordinator*. `jf-note: not clear whether this is a human or an agent...`
...
```jf-note:
i'm not sure the aboe is true.
```

## The populated *example*.

```jf-note:
should use the same meridian example all the way through...
```
