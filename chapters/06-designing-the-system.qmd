...
The Accountability Chart is the tool that maps who owns what inside your company. Each row names one accountability and the person responsible for it. One accountability, one name. No shared rows, no "the team." If you run EOS, you already have one: it maps your seats from Integrator down, with every role and its owner named. If you don't run EOS, think of it as a one-page org chart organized around outcomes rather than titles. Not "Director of Marketing" but "Generate qualified pipeline." Not "Operations Manager" but "Deliver client work on time and on scope." Every meaningful accountability your company has, with one name next to it.

The Hybrid Accountability Chart extends that structure to include agent teams. Every row still names one accountability and one human supervisor. But now some rows also name the agent team doing the execution work underneath that supervisor. The accountability doesn't move from human to agent. The ownership stays with the supervisor. What changes is who does the execution work, and the chart makes that visible.
...
Every accountability in a knowledge business is, at some level, an information problem. The project coordinator isn't "managing projects." She's taking information from one system, combining it with information from another system, applying rules she carries in her head, and producing an output that moves to the next person in the chain. The quoting process isn't "creating quotes." It's pulling customer data from the CRM, matching it against pricing tables in the ERP, applying exception rules that live in a spreadsheet on someone's desktop, and assembling the result into a format the customer can read. When you see the work as information flow, the design question changes. The question isn't "what tool should we use?" It's "what information does this work need, where does it live, and how should it move?"

That question produces an *Information Flow Specification*: a spec for how information moves through one accountability. It names:
...
How is this different from the Knowledge Map you built in Source? The Knowledge Map tells you *what* knowledge exists and *where* it lives. The information flow spec tells you *how* that knowledge moves through a specific workflow: the transformations and handoffs.

Most people don't think about their company this way. They start with roles and tasks. The value of the information flow is that it forces you to start with the outcome instead and work backwards. What does this accountability produce? Who receives it? What has to happen for that output to exist? When you answer those questions first, the roles and tasks fall into their correct positions in the flow. You see where the bottlenecks actually live, which parts of the flow an agent can handle, and which parts genuinely require a person.
...
You don't necessarily need all five. Most first Sprints use patterns one and two, with some pattern four for the routing work. The point is knowing what's available so the design matches the information problem — not the other way around.
...
The fourth question on the HAC is the one operators get wrong most often. They treat it as a technology decision. It's a design decision.
...
**Start AI-assisted**, always. Humans in the loop, reviewing every output, conservative by default. This is the starting position for every agent.
...
1. **System prompt.** The operating rules. Who the agent is, what it's accountable for, what it does and doesn't decide. Three to five sentences, written as if you were briefing a new hire.
...
That last field comes from Julie Bedard and colleagues at BCG. Their March 2026 piece in *Harvard Business Review*, "When Using AI Leads to 'Brain Fry,'" found productivity inverts after three concurrent AI agents per supervisor. The norm they recommend, the same way operations leaders already manage spans of control for human reports, is no more than three high-oversight agents under one supervisor at a time. Before the mini-specs leave Design, count the agents one supervisor is taking on. If it's more than three, the design isn't done; either consolidate agents, raise some toward Automated, or split the supervisor role.

Next to the mini-spec, post the AWS supervisor-capability check: the supervisor has to be able to *trace decisions, challenge outputs, and apply expertise*. If any of the three is missing for an agent on the chart, that agent isn't ready to run. The mini-spec template gets the full walkthrough in the next chapter inside the Design Brief; what matters here is naming the discipline: every agent on the chart has a spec, every spec answers six questions, no agent goes to Build without one.

## *Tool and platform* selection.

The mini-spec names what each agent is. Tool selection names what each agent runs on. That decision belongs in Design, not Build. The design chooses the platform; Build executes against the chosen platform. A team that picks a vendor or tool because of an existing relationship, or because someone read a launch post that week, is making a design decision under the cover of Build, and they will pay for it.

The platform options group into three categories. Most Compound Sprints land in one of them; a few combine two.
...
{{< excalidraw ch06-platform-decision "Decision flowchart: three platform categories (off-the-shelf, low-code, hand-built) with qualifying questions routing each agent in the mini-spec to one of them." >}}
...
Our marketing lead's four-agent setup, the one from Chapter 3, didn't require a line of code. It was a low-code build: skills wired, projects connected, tools configured. The project management agent team that replaced our coordinator role was different. It lives on a server, runs continuously, and gets and receives information in real time across our projects and communication channels. That one required infrastructure, monitoring, and an always-on deployment. Same Sprint discipline, different platform category. The Design phase named both before Build started either one.

### The current tools.

Tools age faster than books do. The categories above are stable; the products that populate them rotate every few quarters. Check current options when you read this. The table names the categories first, with the products that fit them today as examples.
...
:::{.callout-note}
## Action Step
For every agent in your Hybrid Accountability Chart, name the category (off-the-shelf, low-code, hand-built) and the specific tool. Write the choice in one line per agent and the reason in one sentence. If you can't write the reason without guessing, the design isn't done.
:::
...
Every operator eventually asks: what does the human do when the AI does the work? The Human Orchestrator is the answer. It's a different job.
...
Everyone in a knowledge business will eventually need to become an orchestrator at some level. The skills involved aren't exotic; they're the skills of good management. Context management: knowing what information each agent needs and when. Knowing what good looks like: being able to evaluate output quality without redoing the work yourself. The ability to communicate clearly across different agents and platforms. A working understanding of the tools and information different agents need to be successful. These are the same skills that let you create the conditions for the people you're responsible for to succeed. Orchestration is management extended to a workforce that includes agents — the same discipline, applied to a larger team.
...
This shift requires investment. Upskilling employees to work in an orchestration environment, like learning the available tools, isn't optional. Budget for it. The operations lead who became an orchestrator did not wake up with those capabilities. She developed them over several Sprints with deliberate support.
...
The artifact that comes out of the Design Team's work is a *Design Brief* — the single document that captures the design decisions, stakeholders, systems, and specifications Build will inherit. The next chapter introduces the Design Brief in detail. Chapter 8 then shows how the Build Spec is derived from it: restating the relevant sections at developer-execution depth, and adding the implementation-level content (agent scope, failure modes, build path) that Design does not own.
