...
If it spills past one page you are tracking too much — the constraint forces prioritization and prevents the document from becoming an ignored wiki.

1. Hold the whole document to **one page**. If it doesn't fit, you're tracking too much.
2. Cut to the load-bearing entries: a function only earns a Hybrid Accountability Chart row once a Sprint has actually changed it. Backlog items that no longer reflect reality get dropped, not archived in place.
3. Use one line per entry. If a section needs a paragraph to explain itself, it belongs in the underlying chart or backlog, not on this summary page.

> **Meridian example.** Three Sprints in, Meridian's whole picture — three completed and active constraints, two queued, and a Hybrid Accountability Chart covering quoting, CRM-ERP sync, and onboarding — still fit on a single page. They tracked the design layer, not every task in the shop.

## Update it during the quarterly session, not between sessions
...
## Use it as the standard briefing and the board receipt

A new executive or a board question gets answered from one current, concrete document — not a slide deck assembled from memory.

1. Make the Hybrid Org Today the **onboarding briefing** for any new executive: hand them this one page and they see what people own, what AI handles, what's solved, and what's next.
2. Make it the **answer to the board question** "what have the AI investments produced?" — point at the Completed Sprints section and the constraints they removed, in your own vocabulary, with the quarters attached.
3. Keep it current enough that you'd hand it over **today** without first assembling a slide deck. If you'd need to rebuild it from memory to show it, the document isn't doing its job.

> **Meridian example.** By the end of Sprint 3, Meridian's board question about AI ROI had a one-page answer: Completed Sprints showing the quoting bottleneck and CRM-ERP sync both solved, with the quarters they shipped — concrete and current, not a deck built from memory.
...
- [ ] It was updated **in the quarterly session by the leadership team** — not by one person between meetings.
- [ ] You could hand it over **as-is** as a new-executive briefing or a board receipt, with no slide deck assembled from memory.

This is the page the next quarterly session opens with. Pair it with the Compounding Scorecard, which tracks whether the Rhythm is actually paying off across quarters.
